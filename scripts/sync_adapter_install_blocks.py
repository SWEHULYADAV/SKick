...
- **Verification status:** `{entry.get('verification_status', entry.get('status', 'UNKNOWN'))}`
- **Runtime tier:** `{entry.get('runtime_tier', 'unknown')}`
- **Native Skill support:** `{entry.get('native_skill_support', 'unknown')}`
- **Literal `@SKick`:** `{entry.get('activation', {}).get('literal_at_skick', 'unknown')}`
- **Native activation equivalent:** {entry.get('activation', {}).get('native_equivalent', 'Unknown; use the current host documentation.')}
