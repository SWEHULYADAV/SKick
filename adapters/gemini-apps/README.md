# Gemini Apps Adapter — VERIFIED

## Install
On Gemini Apps Skills, upload either `SKILL.md` or a ZIP containing `SKILL.md` in the ZIP's main/root folder. The generated Gemini Apps ZIP deliberately places `SKILL.md` at archive root with `core/` beside it.

Gemini Apps may restrict some script/network behavior in uploaded Skills. This research system keeps its canonical methodology in Markdown and does not require executable helper scripts to function.
...
- **Manifest platform ID:** `gemini-apps`
...
- **Native activation equivalent:** Use the Gemini Skills UI on an eligible surface.
- **Surfaces covered:** Gemini Apps web/app surfaces that expose Skills
...
- **Prebuilt distribution when applicable:** `gemini-apps-skill.zip`
...
1. Confirm the Skill is visible/enabled in the Gemini Skills UI and run a low-risk trigger test.
