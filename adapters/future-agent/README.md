# Future / Unknown Agent — GENERIC FALLBACK

Identify the runtime before the model. Inspect current official docs for Skills/plugins/project instructions/MCP/subagents/persistent state. If a compatible Agent Skill path exists, reuse the canonical Skill. If only a plugin mechanism exists, create a thin plugin wrapper around the same Skill/core. Otherwise use `../generic/PROMPT.md`.

Never invent a proprietary manifest or install path.
...
- **Manifest platform ID:** `future-agent`
- **Support status:** `HOST_RUNTIME_OR_GENERIC_FALLBACK`
- **Surfaces covered:** future/unknown runtimes
...
1. Never invent install semantics for a new runtime; verify first-party docs before adding an adapter.
