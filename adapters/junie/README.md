# JetBrains Junie Adapter — VERIFIED

SKick uses the current first-party/evidence-backed Skill contract for **JetBrains Junie**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
Use .junie/skills/skick for project scope or .agents/skills/skick for a portable project install. Use ~/.junie/skills/skick or ~/.agents/skills/skick for documented user scope.
...
- **Manifest platform ID:** `junie`
- **Support status:** `VERIFIED`
- **Surfaces covered:** JetBrains Junie
...
- **Project/local destinations:** `.junie/skills/skick/`, `.agents/skills/skick/`
- **User/personal destinations:** `~/.junie/skills/skick/`, `~/.agents/skills/skick/`
...
1. Confirm SKick appears in Junie Skill discovery for the selected project/user scope.
2. Run a low-risk matching task or the current explicit Skill invocation flow.
