# Qoder Adapter — VERIFIED

SKick uses the current first-party/evidence-backed Skill contract for **Qoder**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
Use .qoder/skills/skick for project scope or ~/.qoder/skills/skick for user scope. Qoder IDE/CLI can also install compatible Skills through its Skills UI/CLI; QoderWork uses a separate ~/.qoderwork/skills root.
...
- **Project/local destinations:** `.qoder/skills/skick/`
- **User/personal destinations:** `~/.qoder/skills/skick/`
...
1. Confirm SKick appears in Qoder Installed Skills or the CLI /skills list after reload/restart.
2. Invoke with /skick or a matching low-risk prompt and verify activation.
