# OpenCode Adapter — VERIFIED

OpenCode natively discovers Agent Skills from project `.opencode/skills/`, `.agents/skills/`, `.claude/skills/` and corresponding documented global locations. It loads skill bodies on demand through its native Skill tool.

Use `.opencode/skills/skick/` for an OpenCode-specific project install or `.agents/skills/skick/` for a portable project install.

## Serena
Serena officially lists OpenCode among supported MCP clients. Prefer Serena automatically when connected.

## Superpowers
Superpowers upstream documents OpenCode support. If installed, use it as an optional engineering-method layer and avoid duplicate workflow loading.
...
- **Manifest platform ID:** `opencode`
- **Support status:** `VERIFIED`
- **Surfaces covered:** CLI/TUI, web, IDE integrations that use OpenCode runtime
...
- **Project/local destinations:** `.opencode/skills/skick/`, `.agents/skills/skick/`, `.claude/skills/skick/`
- **User/personal destinations:** `~/.config/opencode/skills/skick/`, `~/.agents/skills/skick/`, `~/.claude/skills/skick/`
...
1. Confirm SKick appears in the native skill tool/discovery list.
2. Check OpenCode skill permissions if it is hidden.
