# Amp Adapter — VERIFIED

SKick uses the current first-party/evidence-backed Skill contract for **Amp**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
User/personal paths:
- `~/.config/agents/skills/skick/`
- `~/.agents/skills/skick/`

## Runtime notes

Use .agents/skills/skick for project scope. For user/machine scope use ~/.config/agents/skills/skick or another path explicitly documented by the active Amp release. Amp can also add Skills from supported sources through its native skill commands.
...
- **User/personal destinations:** `~/.config/agents/skills/skick/`, `~/.agents/skills/skick/`
...
1. Run Amp native Skill listing/discovery and confirm SKick is present.
2. Invoke a low-risk matching task and verify the complete Skill resources load.
