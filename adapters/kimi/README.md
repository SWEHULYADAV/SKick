# Kimi Code Adapter - VERIFIED; Kimi Web FALLBACK

## Kimi Code
Kimi Code supports directory-form Agent Skills with `SKILL.md`. Project roots include `.kimi-code/skills/` and `.agents/skills/`; user roots include `~/.kimi-code/skills/` and `~/.agents/skills/` by default. If `KIMI_CODE_HOME` is explicitly configured, its `skills/` directory is the equivalent branded user root.

Kimi plugins can bundle Skills, agents, MCP servers, hooks and session-start instructions. Keep v1.0 as a plain portable Skill unless Kimi-specific plugin functionality is required.

The canonical two-field frontmatter is intentionally conservative; Kimi-specific fields such as invocation controls/arguments belong in an adapter-generated variant, not the canonical Skill.

## Kimi web/model-only
Do not infer Kimi Code's filesystem/plugin contract from the model or web product name. Use a verified host or the generic prompt fallback.

## Superpowers
Superpowers upstream documents Kimi Code support. If installed, use the narrower engineering workflow and retain SKick research/evidence/safety/provenance controls.
...
- **Manifest platform ID:** `kimi`
- **Support status:** `VERIFIED_HOST_SPLIT`
- **Surfaces covered:** Kimi Code CLI, Kimi model/web fallback
...
- **Project/local destinations:** `.kimi-code/skills/skick/`, `.agents/skills/skick/`
- **User/personal destinations:** `~/.kimi-code/skills/skick/`, `~/.agents/skills/skick/`
- **Prebuilt distribution when applicable:** `kimi-plugin.zip`
...
1. Confirm Kimi Code discovers the skill from the selected tier and that project scope overrides lower scopes when intended.
