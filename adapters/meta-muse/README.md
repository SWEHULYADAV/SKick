# Meta AI / Muse Adapter — GENERIC OR HOST-RUNTIME FALLBACK

Meta Muse Spark is available in Meta AI and through Meta Model API, and can be used in external agent runtimes. A first-party portable custom Skill/plugin import contract for meta.ai was not verified.

Use the generic web prompt on meta.ai. When Muse Spark runs through a coding host such as OpenCode or another API orchestrator, install the SKick for that host runtime.

Do not fabricate a MuseCode-specific manifest or filesystem path without verified product documentation.
...
- **Manifest platform ID:** `meta-muse`
- **Support status:** `UNKNOWN`
- **Verification status:** `UNKNOWN`
- **Runtime tier:** `unknown`
- **Native Skill support:** `unknown`
- **Literal `@SKick`:** `unknown`
- **Native activation equivalent:** No native SKick activation mechanism verified.
- **Surfaces covered:** Meta AI web, Meta Model API, external hosts
...
1. Do not fabricate a Muse-specific manifest or path.

### Availability and limitations

- Current evidence is insufficient to claim native Skill installation or invocation support.
