# Replit Adapter — VERIFIED

SKick uses the current first-party/evidence-backed Skill contract for **Replit**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
Install project Skills under .agents/skills/skick (Replit documentation displays /.agents/skills relative to the project root), through the Skills pane, or via a compatible skills CLI flow.
...
- **Manifest platform ID:** `replit`
- **Support status:** `VERIFIED`
- **Surfaces covered:** Replit
