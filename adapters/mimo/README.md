# MiMoCode Adapter — VERIFIED; MiMo Model HOST-RUNTIME

MiMoCode has a built-in Skill system and supports project overrides at `.mimocode/skills/<name>/SKILL.md`; current releases also discover compatible user Skill roots.

For MiMo models used in another agent, install the host runtime adapter instead. Do not duplicate the research methodology for the model name.

## Serena
Prefer Serena automatically if the MiMoCode/host environment exposes it through MCP or equivalent semantic tooling.
...
- **Manifest platform ID:** `mimo`
- **Support status:** `VERIFIED_HOST_SPLIT`
- **Surfaces covered:** MiMoCode, MiMo models in other hosts
...
- **Project/local destinations:** `.mimocode/skills/skick/`
...
1. Confirm the installed MiMoCode release discovers SKick; otherwise verify through the actual host runtime.
