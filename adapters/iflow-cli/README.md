# iFlow CLI Adapter — ECOSYSTEM-VERIFIED, RECHECK FIRST-PARTY DOCS
...
2. Confirm the active runtime really is **iFlow CLI**.
...
- **Manifest platform ID:** `iflow-cli`
- **Support status:** `VERIFIED`
- **Surfaces covered:** iFlow CLI
...
- **Project/local destinations:** `.iflow/skills/skick/`
- **User/personal destinations:** `~/.iflow/skills/skick/`
...
1. Run `/skills refresh` (or the current equivalent) and confirm SKick is registered.
2. Invoke a low-risk matching task and verify supporting files resolve.
