# Zed Adapter — VERIFIED

SKick uses the current first-party/evidence-backed Skill contract for **Zed**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
Use .agents/skills/skick in a trusted worktree for project scope or ~/.agents/skills/skick for user scope. Keep the Skill directory as a direct child of the Skills root and verify Zed discovery after changes.
...
- **Manifest platform ID:** `zed`
- **Support status:** `VERIFIED`
- **Surfaces covered:** Zed
