...
SKick uses the current first-party/evidence-backed Skill contract for **Pi**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
User/personal paths:
- `~/.pi/agent/skills/skick/`
- `~/.agents/skills/skick/`

## Runtime notes

Use .pi/skills/skick or .agents/skills/skick for a trusted project. For user scope use ~/.pi/agent/skills/skick or ~/.agents/skills/skick. Project Skills must respect Pi trust boundaries.
...
- **Project/local destinations:** `.pi/skills/skick/`, `.agents/skills/skick/`
- **User/personal destinations:** `~/.pi/agent/skills/skick/`, `~/.agents/skills/skick/`
...
1. Confirm Pi discovers SKick in the selected scope.
2. Run a low-risk matching task and verify project trust allows Skill loading.
