# LongCat Adapter — HOST-RUNTIME VERIFIED

LongCat 2.0 is a model integrated with coding/agent harnesses such as Claude Code, OpenClaw, and Hermes. No standalone LongCat Skill/plugin format was verified.

Install the SKick into the host harness. For longcat.ai web without a persistent custom instruction/Skill mechanism, use the generic prompt fallback.
...
- **Manifest platform ID:** `longcat`
- **Support status:** `HOST_RUNTIME_OR_GENERIC_FALLBACK`
- **Surfaces covered:** LongCat models in agent harnesses, web fallback
...
1. Verify the actual host rather than inferring capabilities from the model name.
