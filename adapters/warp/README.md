# Warp Adapter — VERIFIED

SKick uses the current first-party/evidence-backed Skill contract for **Warp**. Preserve the complete SKick directory and verify discovery in the active runtime; file presence alone is not a runtime test.
...
Use .agents/skills/skick or .warp/skills/skick for project scope and ~/.agents/skills/skick (or another currently documented Warp Skill root) for global scope.
...
- **Manifest platform ID:** `warp`
- **Support status:** `VERIFIED`
- **Surfaces covered:** Warp
...
- **Project/local destinations:** `.agents/skills/skick/`, `.warp/skills/skick/`
...
1. Confirm the Skill appears in Warp Agent/Skills discovery and invoke it with /skick or a matching task.
2. For cloud/Oz runs, verify the selected environment actually contains the repository Skill.
