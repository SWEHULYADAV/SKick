# Cursor Adapter - VERIFIED

Cursor supports the open Agent Skills format and the **Agent Plugins** open standard in addition to Cursor-specific plugins.

## Recommended v1.0 distribution
- Portable Skill: use the canonical Agent Skill in a Cursor-supported skill location.
- Portable plugin: use the generated Agent Plugin v1 package with root `plugin.json` and `skills/skick/SKILL.md`.
- Cursor-specific plugin: use `.cursor-plugin/plugin.json` only when Cursor-only rules, agents, commands, hooks, variables, or canvases are actually required.

For local plugin testing, use Cursor's documented local-plugin path and reload the client. Structural validation alone is not a Cursor runtime test.

## Superpowers
If Superpowers is installed through Cursor's supported plugin path, delegate matching engineering-method phases without duplicating SKick research/version/safety/provenance work.
...
- **Manifest platform ID:** `cursor`
- **Support status:** `VERIFIED`
- **Surfaces covered:** desktop IDE, agent chat
...
- **Project/local destinations:** `.cursor/skills/skick/`, `.agents/skills/skick/`, `.claude/skills/skick/`, `.codex/skills/skick/`
- **User/personal destinations:** `~/.cursor/skills/skick/`, `~/.agents/skills/skick/`, `~/.claude/skills/skick/`, `~/.codex/skills/skick/`
- **Prebuilt distribution when applicable:** `shared-agent-skill.zip`

### GitHub source fast path

- Use Cursor Customize/Skills current GitHub import flow when available, otherwise copy the complete Skill to a verified project Skill root.
...
1. Open Cursor Customize > Skills or use Agent slash search and confirm SKick is discovered.
