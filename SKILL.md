...
# SKick v1.0 — Engineering Control Plane
...
- Treat the user's repository/project as ground truth when available. For non-trivial repo/code work, check Serena first and, when available, prefer Serena automatically for semantic repo exploration/editing, symbols and references before broad file reads; use a compact repository context map and an explicit fallback when Serena cannot be used. Detect the real language/framework/toolchain/version before loading language-specific specialists; prefer Skills for fast-moving or demonstrated knowledge gaps, not generic syntax repetition.
...
- Keep project structure simple and ownership-clear. For the preferred Python web profile, treat `app.py` + `backend/` + `frontend/` as a placement invariant, not a full template: backend implementation stays in `backend/`, frontend implementation/assets stay in `frontend/`, and deeper structure is created only when needed.
- For websites/web tools without a stronger project constraint, prefer the host-neutral `Python backend + semantic HTML + CSS + vanilla JavaScript` profile. Keep root implementation to a tiny `app.py`, backend implementation under `backend/`, and frontend implementation/assets under `frontend/`; do not prescribe a deeper tree unless complexity requires it. Escalate frameworks only for evidenced product/ecosystem needs.
